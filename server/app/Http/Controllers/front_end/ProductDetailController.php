...
class ProductDetailController extends Controller {
    ...
    public function getProductDetail(Request $request){
        ...
        foreach($Product as $p){
            if($p->products_type_id){
                $productTypeDetail = ProductsTypeDetail::where('products_type_id',$p->products_type_id)->get();
            }
            if($p->products_type_id1){
                $productTypeDetail1 = ProductsTypeDetail::where('products_type_id',$p->products_type_id1)->get();
            }
            if($p->products_type_id2){
                $productTypeDetail2 = ProductsTypeDetail::where('products_type_id',$p->products_type_id2)->get();
            }
            
        }
        $images = ImagesProducts::where('product_id',$id)->select('images')->limit(15)->get();
        return response()->json([
            ...
            'product_type_details'=>[
                ...
                'type1'=>$productTypeDetail1,
                'type2'=>$productTypeDetail2,
            ],
        ]);
    }
}