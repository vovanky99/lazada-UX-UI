...
class SearchController extends Controller {
    ...
    public function getSearchAll(Request $request){
        ...
        $shopAddress = Shop::class;
        if($order !='' && $cat !='' && $score !=''){
            $pd = Products::select('products.*','discount.number as discount',DB::raw('(SELECT AVG(reviews.review_star) from reviews where reviews.product_id = products.id) as reviews_score'),DB::raw('(SELECT SUM(quantity) from order_products where order_products.product_id = products.id) as sold'),DB::raw(`(SELECT city.name from ward,district,city,shop,address where ward.district_id=district.id and district.city_id=city.id AND shop.id=address.addressable_id AND address.addressable_type ='$shopAddress' AND address.ward_id=ward.id AND shop.id = products.shop_id ) as city_name`))->join('discount','discount.id','=','products.discount_id')->whereIn('products.categories_id',explode(',',$cat))->where(DB::raw('(price-(price*(discount.number/100)))'),'>=',$priceTo)->where(DB::raw('(price-(price*(discount.number/100)))'),'<=',$priceFrom)->where('products.title','LIKE',$title.'%')->groupBy('products.id')->having('reviews_score','>=',$score)->orderBy(DB::raw('(price-(price*(discount.number/100)))'),$order)->get();
        }
         else if($order !='' && $cat !=''){
            ...
            // when you have score
            $pd = Products::select('products.*','discount.number as discount',DB::raw('(SELECT AVG(reviews.review_star) from reviews where reviews.product_id = products.id) as reviews_score'),DB::raw('(SELECT SUM(quantity) from order_products where order_products.product_id = products.id) as sold'),DB::raw(`(SELECT city.name from ward,district,city,shop,address where ward.district_id=district.id and district.city_id=city.id AND shop.id=address.addressable_id AND address.addressable_type ='$shopAddress' AND address.ward_id=ward.id AND shop.id = products.shop_id ) as city_name`))->join('discount','discount.id','=','products.discount_id')->where(DB::raw('(price-(price*(discount.number/100)))'),'>=',$priceTo)->where(DB::raw('(price-(price*(discount.number/100)))'),'<=',$priceFrom)->where('products.title','LIKE',$title.'%')->groupBy('products.id')->having('reviews_score','>=',$score)->orderBy(DB::raw('(price-(price*(discount.number/100)))'),$order)->get();
        }
        else if($cat !='' && $score !=''){
